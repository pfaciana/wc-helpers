...
class Variation_Data_Group
{
	...
	protected function addGroup ( $group )
	{
		...
		add_action( $this->getActionTag( $group ), function ( $loop, $variation_data, $variation ) use ( $group ) {
			$group['wrapper_class'] = $this->getClassString( $group['wrapper_class'] );
			$group['conditionals']  = $this->getClassString( $group['conditionals'] );
			?>
			<div id="<?= $this->id ?>_variation_data_group" class="variation_data_group <?= $group['wrapper_class'] ?> <?= $group['conditionals'] ?>">
	 ...
	 *                           Signature: $callback( array $group, int $loop, array $variation_data, WC_Product_Variation $variation )
	...
	{
		add_action( $this->getActionTag( [ 'group' => $this->id ] ), function ( $loop, $variation_data, $variation ) use ( $callback ) {
